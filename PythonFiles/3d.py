...
wall_left = pygame.math.Vector3(100, 0, -20)
wall_right = pygame.math.Vector3(125, 0, 30)
...
player = pygame.math.Vector3(-100, 0, 0)
player_angle = 0
player_speed = 0.1
...
fov = 60
fov_scaling_factor = 1 / tan(radians(60)/2)
...
def move_player(keys):
    ...
    if keys[pygame.K_ESCAPE]:
        running = False
    # Stop player from crossing borders
    player.x = max(-400, player.x)
    player.x = min(400, player.x)
    player.z = max(-150, player.z)
    player.z = min(150, player.z)
...
def main():
    ...
    while running:
        ...
        #                   2D
        # Draw wall
        pygame.draw.line(screen1, line_color, (wall_left.x + 400, -wall_left.z + 150), (wall_right.x + 400, -wall_right.z + 150))
        # Draw player
        pygame.draw.circle(screen1, player_color, (player.x + 400, -player.z + 150), 5)
        pygame.draw.line(screen1, player_color,
            (player.x + 400, -player.z + 150),
            (player.x + 400 + (20 * cos(player_angle)), -player.z + 150 + (20 * sin(player_angle))))
        
        #                   2D (View Space)
        # Transform wall relative to player
        wall_left_trans = pygame.math.Vector3(wall_left.x - player.x, 0, wall_left.z - player.z)
        wall_right_trans = pygame.math.Vector3(wall_right.x - player.x, 0, wall_right.z - player.z)
        ...
        wall_left_trans = pygame.math.Vector3(
            wall_left_trans.x * sin(player_angle) - wall_left_trans.y * cos(player_angle),
            wall_left_trans.y,
            wall_left_trans.x * cos(player_angle) + wall_left_trans.y * sin(player_angle)
        )
        wall_right_trans = pygame.math.Vector3(
            wall_right_trans.x * sin(player_angle) - wall_right_trans.y * cos(player_angle),
            wall_right_trans.y,
            wall_right_trans.x * cos(player_angle) + wall_right_trans.y * sin(player_angle)
        )
        # Draw wall
